...
FROM
  `ride`
  JOIN `connection` ON `connection`.`id` = `ride`.`connection_id`
  JOIN `station` AS `departure` ON `departure`.`id` = `connection`.`departure_id`
  JOIN `station` AS `arrival` ON `arrival`.`id` = `connection`.`arrival_id`
ORDER BY
  `ride`.`date` DESC,
